...
void main()
{
	...
	sigaddset(&act.sa_mask,6);//add 2 and 6 bits
	sigaddset(&act.sa_mask,2);
	sigprocmask(SIG_BLOCK,&act.sa_mask,0);//block 2 and 6 bits
	sigaction(2,&act,0);//changing the behavior of 2 
	...
	sigdelset(&act.sa_mask,2);//2 is in block state
	//sigdelset(&act.sa_mask,2);//when 2 is block the sigalrm 14 is unblocked 
	sigprocmask(SIG_UNBLOCK,&act.sa_mask,0);//unblocked signal 6
//	alarm(3);//alarm perform after 3sec and terminate the process when 6 is unblocked
	sleep(5);
	i=0;
	while(i<=20)
	{
